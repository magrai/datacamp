...
class TestBlock:
    ...
    def test_path(self):
        G=networkx_v1x.path_graph(6)
        partition=[[0,1],[2,3],[4,5]]
        M=networkx_v1x.blockmodel(G,partition)
    ...
    def test_multigraph_path(self):
        G=networkx_v1x.MultiGraph(networkx_v1x.path_graph(6))
        partition=[[0,1],[2,3],[4,5]]
        M=networkx_v1x.blockmodel(G,partition,multigraph=True)
        assert_equal(sorted(M.nodes()),[0,1,2])
        assert_equal(sorted(M.edges()),[(0,1),(1,2)])
        for n in M.nodes():
            assert_equal(M.node[n]['nedges'],1)
    ...
    def test_barbell(self):
        G=networkx_v1x.barbell_graph(3,0)
        partition=[[0,1,2],[3,4,5]]
        M=networkx_v1x.blockmodel(G,partition)
