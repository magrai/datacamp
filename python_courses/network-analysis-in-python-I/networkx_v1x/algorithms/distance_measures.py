...
#        nodes=v
    ...
    for n in G.nbunch_iter(v):
        if sp is None:
            length=networkx_v1x.single_source_shortest_path_length(G,n)
            L = len(length)
        else:
            try:
                length=sp[n]
                ...
                raise networkx_v1x.NetworkXError('Format of "sp" is invalid.')
        if L != order:
            msg = "Graph not connected: infinite path length"
            raise networkx_v1x.NetworkXError(msg)
