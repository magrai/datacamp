...
from networkx_v1x import edge_current_flow_betweenness_centrality \
    as edge_current_flow

from networkx_v1x import approximate_current_flow_betweenness_centrality \
    as approximate_cfbc
...
class TestFlowBetweennessCentrality(object):
    ...
    def test_K4_normalized(self):
        ...
        b=networkx_v1x.current_flow_betweenness_centrality(G,normalized=True)
        b_answer={0: 0.25, 1: 0.25, 2: 0.25, 3: 0.25}
        for n in sorted(G):
            assert_almost_equal(b[n],b_answer[n])
        G.add_edge(0,1,{'weight':0.5,'other':0.3})
        b=networkx_v1x.current_flow_betweenness_centrality(G,normalized=True,weight=None)
        for n in sorted(G):
            assert_almost_equal(b[n],b_answer[n])
        ...
        b=networkx_v1x.current_flow_betweenness_centrality(G,normalized=True,weight='other')
        for n in sorted(G):
            assert_almost_equal(b[n],wb_answer[n])
    ...
    def test_K4(self):
        ...
        for solver in ['full','lu','cg']:
            b=networkx_v1x.current_flow_betweenness_centrality(G, normalized=False, 
                                                           solver=solver)
...
class TestApproximateFlowBetweennessCentrality(object):
    ...
    def test_grid(self):
        ...
        G=networkx_v1x.grid_2d_graph(4,4)
        b=networkx_v1x.current_flow_betweenness_centrality(G,normalized=True)
        epsilon=0.1
        ba = approximate_cfbc(G,normalized=True, epsilon=0.5*epsilon)
        for n in sorted(G):
